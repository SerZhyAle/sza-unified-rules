...
Closing gates: a transition INTO `Implemented` or `Verified` runs `check-evidence-durable.ps1` (a closed spec must not cite evidence under disposable `temp/`, S1606) and `check-open-items-carried.ps1` (a closed spec must not strand an open research item without a `Carrier: Sxxxx`, S1607). A transition INTO `Verified` additionally runs `check-audit-recorded.ps1` (the spec must carry a non-empty `## Last Audit` block, S2298). All are invoked from `Assert-ClosingGates` in `_lib.ps1`, which `update.ps1`, `close.ps1` and `bulk-update.ps1` all call, so no status-change path bypasses them. One checker judges the other direction: a transition OUT of `BlockNeedUserTest` runs `check-probe-absent.ps1` (no probe of the ticket may remain in source, S2934), placed before the early return that skips an ungated `$NewStatus` - that return was the hole, because `BlockNeedUserTest -> In Progress` names no gated status and so reached no checker at all while the probes stayed in the tree. The refusal prints `probes.removeCommand` filled in with the id rather than deleting anything: a catalog mutator writing source would do it outside any code-domain lock and outside the closure that judges a source edit. `Archived` is deliberately not gated, and `archive.ps1` - the path a release sweep takes - does not call this function at all, which is why the exit gate reaches only a hand-written `Archived` write and not the sweep; the sweep deletes the probes of everything it archives and proves it with the tree gate. The entry gate `check-probe-present.ps1` asks a second question of every probe it finds (S2934): whether it owns its physical line, judged by `probes.ownLineRegex`, because a probe is deleted in bulk when the ticket leaves the status. A malformed probe does not satisfy presence - split across two checkers, presence would answer PASS about the line shape answers FAIL about. The predicate is a profile key and not a library function so that a consuming repository can adopt it without waiting for a plugin deploy: an unknown key in the project profile is carried into the merged tree, an undefined function is not. A re-affirmation - the same status written over itself, the shape a `-StatusNote` replacement takes - runs exactly one checker, `check-probe-present.ps1`, and only when that status is `BlockNeedUserTest` (S2656): every other checker in this function asks what the ticket ACHIEVED or what its own spec file says, which was answered at the entry transition and does not decay on its own, while the probe's subject is the source tree, which changes under a standing ticket by other hands. Re-judging the rest would turn a housekeeping rewrite into a refusal - sharpest for `check-audit-current.ps1`, whose fingerprint would meet a re-close done purely to repair a diverged header with an audit refusal instead of the repair. The third gate is `Verified`-only on purpose: `Verified` asserts that an audit passed and the verdict of that audit exists nowhere but the block, while `Implemented` asserts only that the code is done - true before any audit has run, so gating it would demand a verdict ahead of the run producing one. The research-section parse behind the second gate lives in `_research-items.ps1`, a leaf file `_lib.ps1` and `preview.ps1` both dot-source, so `preview.ps1`'s `research_open_count` / `research_uncarried_count` report exactly what the gate will enforce rather than a second opinion about it (S1621). The third gate's heading pattern lives in the same file for the same reason, and `preview.ps1`'s `last_audit_present` reads it: matched by heading TEXT, so the numbered form `## 6. Last Audit` counts, where the literal test it replaced reported that spec as unaudited in both places at once. The lifecycle-status sets live in `_status-sets.ps1` for the same reason: `Test-BlockerReleasedStatus` defines which statuses release a dependent (`Implemented`, `Verified`, `BlockNeedUserTest`, `Archived` - the set `RELEASE_READY.md` uses plus `Archived`) and is shared by `preview.ps1` and `_lib.ps1` so the preflight verdict and release-file routing answer from one definition (S1864).
...
- Section shape (S2852): the release package is a **heading line holding the number and nothing else** (`37`, or `--` for unscheduled), and every ticket row below it belongs to that package until the next heading. The owner sorts these files by hand, so re-packaging a ticket has to be a plain line move with nothing on the row to keep in step. `RELEASE_QUEUE_DONE.md` uses its shipped-block title (`## release 36 - v.. - ..`) as the heading instead, since it already names the package.
- Line shape: `ticket  changed  status`. `ticket` = the spec file name without its extension. `changed` = `YY-MM-DD HH:mm`, when the STATUS last moved, not when the spec text was edited - a ticket can move status several times in one working day and a date alone cannot order those moves. The open package is named by the `current-next-release:` marker; the `DEBUG-v0NN` branch only labels it, so the two may differ by one or more after a package ships without a branch (a Wear release does exactly that).
- Reading tolerates the pre-S2852 shape - a leading package column, a `yyyy-MM-dd` date with no time - so a half-migrated or stale copy still parses. Writing emits only the current shape, so the first write normalises whatever it read, and `Write-ReleaseFile` emits a missing heading whenever the package changes under it, which is what keeps read/write/read closed.
...
- Ownership split: the catalog owns `status`, these files own the package sections and line order. **A script may add a ticket, refresh its status/date, move a line to the sibling file, or drop one that left the active journal - it must never reorder lines or re-home a row into another package.** A ticket keeps its package when it crosses between files.
