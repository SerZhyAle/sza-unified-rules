...
- The contract binds only the names a **program** addresses: the three agent-rules filenames,
  `docs/contracts/CONTRACT_*.md` as pointers, the uppercase type prefixes, `README.md` and `LICENSE` at
  the root, and the ledger sitting where the stamp says. Everything else in the layout page is a target
  shape a repository adapts, and adapting it breaks nothing.
