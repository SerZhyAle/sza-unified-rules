...
- Prefer the project's own wrapper scripts over hand-rolled git/gh invocations with fragile nested
  quoting.
- **Three of these ship as hooks with the `sza` plugin**, in [`hooks/`](../hooks/README.md) - the `find`
  guard, the command-head guard, and the fire-and-forget guard that enforces
  [AI_USAGE.md](AI_USAGE.md) section 1's forbidden half. Each blocks the call before the shell spawns, in
  every repository, not only in canon adopters. A project does not need its own copy; see
  [AI_USAGE.md](AI_USAGE.md) section 5 for why an unenforced version of any of them is worth roughly nothing.
